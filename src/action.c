	...
	for ( uint32_t i = 0; i < _action_currentLength; i++ )
	{
		...
		switch ( actionNameToId ( actionName ) )
		{
			...
			case aT(dyna):
			{
				...
				char * id = NULL;
				if ( !jsonGet ( _action_current[ i ].params[ j ], 0, "id", (void*)&id, &type ))
				{
				...
				char * vitesse = NULL;
				if ( !jsonGet ( _action_current[ i ].params[ j ], 0, "Vitesse", (void*)&vitesse, &type ))
				{
			...
			{ // done
				...
				char * tolerance = NULL;
				if ( !jsonGet ( _action_current[ i ].params[ j ], 0, "Tolerance", (void*)&tolerance, &type ))
				{
